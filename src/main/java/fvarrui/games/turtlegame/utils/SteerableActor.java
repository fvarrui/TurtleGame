...
public class SteerableActor extends TexturedActor implements Steerable<Vector2> {

	public static enum SteeringState {
		WANDER, SEEK, FLEE, ARRIVE, NONE
	} // a list of possible behaviours
	...
	float zeroThreshold = 0.1f; // how accurate should checks be (0.0000001f will mean the entity must get
								// within 0.0000001f of
								// target location. This will cause problems as our entities travel pretty fast
								// and can easily over or undershoot this.)
	...
	private static final SteeringAcceleration<Vector2> steeringOutput = new SteeringAcceleration<Vector2>(
			new Vector2()); // this is the actual steering vactor for our unit
	private float boundingRadius = 1f; // the minimum radius size for a circle required to cover whole object
	private boolean tagged = true; // This is a generic flag utilized in a variety of ways. (never used this
									// myself)
	private boolean independentFacing = false; // defines if the entity can move in a direction other than the way it
												// faces)


	public SteerableActor(String texturePath) {
		super(texturePath);
	}

	public SteerableActor(Texture texture) {
		super(texture);
	}

	public SteerableActor(TextureRegion textureRegion) {
		super(textureRegion);
	}
